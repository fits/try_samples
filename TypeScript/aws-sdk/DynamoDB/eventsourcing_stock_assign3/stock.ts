...
import { 
    ...
    ExecuteStatementCommand, AttributeValue, QueryCommand
...
export class StockAction {
    ...
    static async cancelAssign(stockId: StockId, assignId: AssignId): Promise<StockEvent> {
        const assignedQty = await StockAction.currentAssignedQty(stockId, assignId)

        if (assignedQty <= 0) {
            throw new Error('not found assign')
        }
        ...
        const event: StockAssignCancelled = {
            tag: 'stock.assign-cancelled',
            stockId,
            revision: rev,
            assignId,
    ...
    static async find(stockId: StockId): Promise<Stock | undefined> {
        ...
        const qty = parseInt(es.Items?.[0].Qty.N ?? '0')
        const assignedQty = parseInt(es.Items?.[0].AssignedQty.N ?? '0')
        const freeQty = parseInt(es.Items?.[0].FreeQty.N ?? '0')

        const rev = es.Items?.[0].Revision.N ?? ''
    ...
    static async restore(stockId: StockId): Promise<Stock | undefined> {
        ...
        return events.reduce((acc, ev) => {
            ...
            acc.freeQty = acc.qty - acc.assignedQty

            return acc
        }, stock)
    }

    private static async currentAssignedQty(stockId: StockId, assignId: AssignId): Promise<Quantity> {
        const rs = await client.send(new QueryCommand({
            TableName: eventTable,
            IndexName: eventIndex,
            KeyConditionExpression: 'StockId = :sid AND AssignId = :aid',
            ExpressionAttributeValues: {
                ':sid': { S: stockId },
                ':aid': { S: assignId }
            },
        ...
        return (rs.Items ?? []).reduce(
            (acc, r) => acc + parseInt(r.AssignedQty?.N ?? '0'),
            0
        )
    }
    ...
    private static toItem(event: StockEvent): DataItem | undefined {
        switch (event.tag) {
            case 'stock.assigned':
            case 'stock.assign-cancelled':
                return {
        ...
        switch (item.Tag.S) {
            ...
            case 'stock.assign-cancelled':
                return {
                    ...
                    assignedQty: parseInt(item.AssignedQty.N ?? '0')
                }
            case 'stock.arrived':
                return {
                    tag: item.Tag.S,
                    stockId: item.StockId.S ?? '',
                    revision: item.Revision.N ?? '',
                    incoming: parseInt(item.Incoming.N ?? '0')
                }
        }
        return undefined
    }
