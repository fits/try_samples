...
const websocketKey = websocket => websocket.upgradeReq.headers['sec-websocket-key']

const subscriptions = {
    onConnect: (params, websocket, ctx) => {
        console.log(`*** subscription connect: ${websocketKey(websocket)}`)
    },
    onDisconnect: (websocket, ctx) => {
        console.log(`*** subscription disconnect: ${websocketKey(websocket)}`)
    }
}
