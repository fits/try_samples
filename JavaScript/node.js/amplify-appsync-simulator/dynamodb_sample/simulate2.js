...
simulator.getDataLoader('ItemTable').load = (req) => {
    console.log(`*** dataSource load`)
    console.log(req)

    const id = req.key.id.S

    if (req.operation == 'PutItem') {
        items[id] = { id, value: parseInt(req.attributeValues.value.N) }
    }
