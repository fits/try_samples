...
public class SkipAndTakeSample {
	...
	private static void skipAndTakeSampleA(Environment env) throws Exception {
		...
		Promise<String> promise = skip(stream, 2).take(3).observe(System.out::println).next();
	...
	private static void skipAndTakeSampleB(Environment env) throws Exception {
		...
		TapAndControls<String> tap = skip(stream, 2).take(3).observe(System.out::println).tap();
