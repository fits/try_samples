...
object QProductVariation extends QProductVariation("productVariation") {
  override def as(variable: String) = new QProductVariation(variable)
  
}

class QProductVariation(md: PathMetadata) extends RelationalPathImpl[ProductVariation](md, null, "product_variation") {
  ...
  val primary: PrimaryKey[ProductVariation] = createPrimaryKey(id)
