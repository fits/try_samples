...
object QProduct extends QProduct("product") {
  override def as(variable: String) = new QProduct(variable)
  
}

class QProduct(md: PathMetadata) extends RelationalPathImpl[Product](md, null, "product") {
  ...
  val primary: PrimaryKey[Product] = createPrimaryKey(id)
