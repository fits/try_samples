...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let n = train_idx.size1()?;

    for epoch in 1..=50 {
        let mut batch_loss = 0.0f64;

        for bidx in Tensor::randperm(n, (Kind::Int64, device)).split(5, 0) {
            let batch_data = train_data.index_select(0, &bidx);
            let batch_labels = train_labels.index_select(0, &bidx);
