...
impl StockMoveAction {
    pub fn move_id(&self) -> StockMoveId {
        match self {
            Self::Start(StartCommand { move_id, .. }) |
            Self::Assign(AssignCommand { move_id, .. }) |
            Self::Cancel(CancelCommand { move_id, .. }) |
            Self::Shipment(ShipmentReport { move_id, .. }) |
            Self::ShipmentFailure(ShipmentFailureReport { move_id, .. }) |
            Self::Arrival(ArrivalReport { move_id, .. })
                => move_id.clone(),
        }
    }
