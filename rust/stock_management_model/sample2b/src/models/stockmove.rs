...
fn start() {
    ...
    let r = StockMove::start(
        &StockMove::initial_state(), 
        item1.clone(), 1, from1.clone(), to1.clone()
    );
...
fn start_with_zero_qty() {
    ...
    let r = StockMove::start(
        &StockMove::initial_state(),
        item1.clone(), 0, from1.clone(), to1.clone()
    );
    assert!(r == None);
}
...
fn assign_after_draft_with_stock() {
    ...
    let stock = Stock::managed_new(item1.clone(), from1.clone()).update_qty(5);

    let r = state.assign(&stock);
...
fn assign_after_draft_with_zero_stock() {
    ...
    let stock = Stock::managed_new(item1.clone(), from1.clone());
...
#[test]
fn assign_after_draft_with_diff_stock() {
    let item1 = "item-1".to_string();
    let from1 = "from-1".to_string();
    let to1 = "to-1".to_string();
    ...
    let stock = Stock::managed_new(item1.clone(), to1.clone()).update_qty(5);
