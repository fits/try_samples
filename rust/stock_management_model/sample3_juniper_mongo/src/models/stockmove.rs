...
pub enum StockMoveAction {
    Start { item: ItemCode, qty: Quantity, from: LocationCode, to: LocationCode },
    Complete,
    Cancel,
    Assign { stock: Stock },
    Ship { outgoing: Quantity },
    Arrive { incoming: Quantity },
}
...
impl StockMove {
    ...
    pub fn action(&self, action: StockMoveAction) -> MoveRes {
        match action {
            StockMoveAction::Start { item, qty, from, to } => 
    ...
    fn start(&self, item: ItemCode, qty: Quantity, 
        from: LocationCode, to: LocationCode) -> MoveRes {
