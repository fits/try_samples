        ...
        match self {
            Self::Whole(m) => match m {
                DiscountMethod::ValueDiscount(v) => {
                    ...
                    if v > Amount::zero() {
                        Some(DiscountReward::SingleDiscount(v, RewardTarget::Group(items), m.clone()))
                    } else {
                        None
                    }
                }
                DiscountMethod::RateDiscount(r) => {
                    ...
                    if d > Amount::zero() {
                        Some(DiscountReward::SingleDiscount(d, RewardTarget::Group(items), m.clone()))
                    } else {
                        None
                    }
                }
                DiscountMethod::ChangePrice(p) => {
                    ...
                    if total > price {
                        Some(DiscountReward::SingleDiscount(price, RewardTarget::Group(items), m.clone()))
                    } else {
                        None
                    }
                }
                                ...
                                let rs = items
                                    ...
                                    .map(|(i, x)| {
                                            ...
                                            || x.price <= Amount::zero()
                                        {
                                            ...
                                            count += 1;
                                            (Some(v.clone().min(x.price.clone())), RewardTarget::Single(x))
                                        }
                                    })
                                    .collect::<Vec<_>>();
                                ...
                                let rs = items
                                    ...
                                    .map(|(i, x)| {
                                            ...
                                            || x.price <= Amount::zero()
                                        {
                                            ...
                                            count += 1;
                                            (Some(r.clone() * x.price.clone()), RewardTarget::Single(x))
                                        }
                                    })
                                    .collect::<Vec<_>>();
