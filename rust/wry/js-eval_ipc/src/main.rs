...
<body>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', (ev) => {
            ipc.postMessage(`DOMContentLoaded: ${ev}`)
        })

        window.addEventListener('click', (ev) => {
            ipc.postMessage(`click: ${ev}`)
        })
        ...
        function getSize() {
            return {
                physical: {
                    ...
                    height: {{ state.physical_size.height }}
                },
                logical: {
                    width: {{ state.logical_size.width }},
                    height: {{ state.logical_size.height }}
                }
            }
        }
    </script>
        ...
        if let Some(window) = &self.window {
            match event {
                ...
                WindowEvent::MouseInput { button, state, .. } if state.is_pressed() => {
                    ...
                    if let Some(view) = &self.webview {
                        let r = view.evaluate_script_with_callback("getSize()", |s| {
                            println!("getSize = {s:}");
                        });

                        println!("evaluate_script_with_callback: {r:?}");
                    }
                }
                _ => {}
            }
