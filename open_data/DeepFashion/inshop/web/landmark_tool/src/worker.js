...
onmessage = ev => {
    switch (ev.data.type) {
        case 'init':
            ...
            break
        case 'predict':
            const outputLayer = model.outputLayerNames[0]

            const shape = model.modelLayersMap.get(outputLayer).output.tensor.shape

            const data = {}
            data[model.inputLayerNames[0]] = ev.data.input

            Promise.resolve(model.predict(data))
                .then(r => new KerasJS.Tensor(r[outputLayer], shape))
                .then(summary)
                .then(r => {
