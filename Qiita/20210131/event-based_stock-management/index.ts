...
class Store {
    ...
    private eventSeqCol: Collection
    private stocksCol: Collection

    constructor(eventsCol: Collection, eventSeqCol: Collection, stocksCol: Collection) {
        this.eventsCol = eventsCol
        this.eventSeqCol = eventSeqCol
    ...
    private async nextId(): Promise<EventId> {
        const res = await this.eventSeqCol.findOneAndUpdate(
            { _id: 'seq_no' },
            { '$inc': { seq_no: 1 } },
            { upsert: true, returnOriginal: false }
        )

        return res.value.seq_no
    }
}
...
const resolvers = {
    ...
    Mutation: {
        ...
        arrive: async(parent, { id, incoming }, { store }, info) => {
            ...
            return doMoveAction(
                store, rs, id, 
                { tag: 'stock-move.arrive', incoming }
            )
        },
        complete: async(parent, { id }, { store }, info) => {
            const rs = await store.loadMove(id)

            return doMoveAction(store, rs, id, { tag: 'stock-move.complete' })
        },
        cancel: async(parent, { id }, { store }, info) => {
            const rs = await store.loadMove(id)

            return doMoveAction(store, rs, id, { tag: 'stock-move.cancel' })
        }
    }
}
