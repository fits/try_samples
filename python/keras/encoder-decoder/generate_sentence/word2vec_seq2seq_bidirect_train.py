...
from keras.layers import Input, Dense, GRU, add
...
wv_model = word2vec.Word2Vec(keywords + sentences, wv_size, min_count = 1, iter = wv_iter)
...
encoder = Bidirectional(GRU(n_hidden, return_state = True))
enc_inputs = Input(shape = (None, wv_model.vector_size))
enc_outputs, enc_fw_h, enc_bw_h = encoder(enc_inputs)

enc_states = [add([enc_fw_h, enc_bw_h])]
